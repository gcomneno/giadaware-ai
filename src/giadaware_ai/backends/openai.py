    ...
    def generate_json(
        ...
        *,
        ...
        try:
            ...
            raise AIUnavailableError(
                f"OpenAI is unavailable: {exc.reason}"
            ) from exc
        ...
        if not isinstance(envelope, dict):
            raise AIInvalidResponseError(
                "OpenAI response envelope must be a JSON object"
            )
        ...
        if not isinstance(output, list):
            raise AIInvalidResponseError(
                "OpenAI response has no output list"
            )
        ...
        for item in output:
            ...
            for part in content:
                ...
                if not isinstance(text, str):
                    raise AIInvalidResponseError(
                        "OpenAI output_text.text is not a string"
                    )
                text_parts.append(text)

        if not text_parts:
            raise AIInvalidResponseError(
                "OpenAI response has no assistant output_text"
            )
