...
class OpenAIBackendTests(unittest.TestCase):
    ...
    def test_generate_json_without_schema_uses_json_object_mode(self):
        backend = OpenAIBackend(api_key="secret")
    ...
    def test_invalid_configuration_fails_before_network(self):
        ...
        with self.assertRaises(AIConfigurationError):
            OpenAIBackend(api_key="secret", model="   ")

        with self.assertRaises(AIConfigurationError):
            OpenAIBackend(api_key="secret", timeout=0)

    def test_non_mapping_schema_is_configuration_error(self):
        backend = OpenAIBackend(api_key="secret")
    ...
    def test_non_serializable_schema_is_configuration_error(self):
        backend = OpenAIBackend(api_key="secret")
    ...
    def test_incomplete_response_is_invalid_response(self):
        backend = OpenAIBackend(api_key="secret")
        response = io.StringIO(
            json.dumps({"status": "incomplete", "output": []})
        )
    ...
    def test_missing_output_text_is_invalid_response(self):
        backend = OpenAIBackend(api_key="secret")
        response = io.StringIO(
            json.dumps({"status": "completed", "output": []})
        )
